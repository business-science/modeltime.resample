...
forecast_panel_tbl %>%
  ...
  labs(title = "Forecast Plot for Panel Data Predictions",
       caption = "Prophet with Regressors is struggling with Panel Data. Boosted models are doing well.")
```
...
__Resample Plot__ We can visually evaluate the accuracy with `plot_modeltime_resamples()`. We can see that Prophet Boost has the lowest average RMSE compared to the other 2 models. 

```{r}
resample_results %>%
  plot_modeltime_resamples(.summary_fn = mean, .interactive = FALSE)
```




__Accuracy Table__ We can get an interactive or static table using `modeltime_resample_accuracy()`.
